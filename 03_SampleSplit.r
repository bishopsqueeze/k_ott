...
set.seed(4321)
gbmFit1 <- train(   x=train.is,
                    y=class.is,
                    method = "gbm",
                    tuneGrid=data.frame(n.trees=100, interaction.depth=2, shrinkage=0.1),
                    trControl = fitControl,
                    metric = "logloss")


##------------------------------------------------------------------
## Test #2 :: Use the vanilla features + Distance feature (logloss = 0.663391 +- 0.002061915)
##------------------------------------------------------------------

train           <- cbind(train.feat, train.distFeat.unif)
train.is        <- train[ idx7030, ]
train.os        <- train[-idx7030, ]
class.is        <- train.target[ idx7030]
class.os        <- train.target[-idx7030]
